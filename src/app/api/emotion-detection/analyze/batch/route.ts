...
export async function POST(request: NextRequest) {
  try {
    ...
    if (body.process_all) {
      ...
      delete body.process_all; // Remove process_all flag before sending to emotion detection API
    }
    
    // Log the payload being sent for debugging
    console.log("Batch request payload:", JSON.stringify(body, null, 2));

    const response = await fetch(`${EMOTION_DETECTION_URL}/api/v1/emotion/analyze/batch`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { error: errorData.detail || `Failed to analyze emotion batch: ${response.statusText}` },
    ...
    console.error("Error analyzing emotion batch:", error);
    ...
    return NextResponse.json(
      {
        error:
          error instanceof Error ? error.message : "Failed to analyze emotion batch",
      },
      { status: 500 },
    );
  }
